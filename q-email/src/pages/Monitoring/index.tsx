...
import { generateAiRemediation, AiRemediationResponse } from '../../services/groqAiService';
...
export const MonitoringPage: React.FC<MonitoringPageProps> = ({
  ...
  const [aiRemediationMap, setAiRemediationMap] = useState<{ [incidentId: string]: AiRemediationResponse }>({});
  const [loadingAiIncidentId, setLoadingAiIncidentId] = useState<string | null>(null);
  ...
  const handleFetchAiRemediation = async (inc: IncidentDetailItem) => {
    ...
    try {
      ...
      showToast(`AI Remediation error: ${err.message}`);
    } finally {
      setLoadingAiIncidentId(null);
    }
  };
                  ...
                  {selectedIncidentDetail && (
                    <div className="col-span-4 border border-[#E2E8F0] bg-[#FFFFFF] rounded-[2px] overflow-hidden shadow-none flex flex-col h-[620px] max-h-[620px] sticky top-4">
                      ...
                      <div className="flex-1 p-5 space-y-4 font-sans overflow-y-auto min-h-0">
                        ...
                        {/* Automated AI Remediation Assistant Block */}
                        <div>
                          <div className="font-mono text-[9.5px] text-[#75777D] uppercase tracking-wider mb-2 font-medium flex items-center justify-between">
                            <span>AUTOMATED AI REMEDIATION</span>
                            <span className="text-[#0058BE] font-bold">QDS SENTINEL AI</span>
                          </div>

                          {aiRemediationMap[selectedIncidentDetail.id] ? (
                            <div className="bg-[#FAF8FF] border border-[#D8B4FE] p-3.5 rounded-[2px] font-mono text-[11px] space-y-2.5">
                              <div className="flex justify-between items-center pb-1.5 border-b border-[#E9D5FF]">
                                <span className="text-[#6B21A8] font-bold uppercase text-[10px] tracking-wider">REMEDIATION PLAYBOOK</span>
                                <span className="text-[9px] bg-[#6B21A8] text-white px-2 py-0.5 rounded font-bold uppercase">
                                  {aiRemediationMap[selectedIncidentDetail.id].model}
                                </span>
                              </div>
                              
                              {/* Formatted Remediation Steps */}
                              <div className="text-[#091426] text-[11px] font-mono whitespace-pre-wrap leading-relaxed max-h-[180px] overflow-y-auto space-y-1.5 p-1">
                                {aiRemediationMap[selectedIncidentDetail.id].remediationPlan}
                              </div>

                              {/* CLI Execution Box */}
                              {aiRemediationMap[selectedIncidentDetail.id].cliCommands.length > 0 && (
                                <div className="bg-[#091426] border border-[#1E293B] p-2.5 rounded-[2px] font-mono text-[10.5px] space-y-1 mt-2">
                                  <div className="text-[#94A3B8] text-[9px] font-bold uppercase tracking-wider">EXECUTION CLI COMMANDS:</div>
                                  {aiRemediationMap[selectedIncidentDetail.id].cliCommands.map((cmd, cIdx) => (
                            ...
                            <button
                              onClick={() => handleFetchAiRemediation(selectedIncidentDetail)}
                              disabled={loadingAiIncidentId === selectedIncidentDetail.id}
                              className="w-full py-2.5 bg-[#6B21A8] hover:bg-[#581C87] text-white font-mono text-[11px] font-bold uppercase tracking-wider rounded-[2px] transition-colors cursor-pointer flex items-center justify-center gap-2"
                            >
                              <Zap className="w-3.5 h-3.5 text-[#FDE047]" />
                              <span>{loadingAiIncidentId === selectedIncidentDetail.id ? 'GENERATING REMEDIATION PLAYBOOK...' : 'PROVIDE REMEDIATION'}</span>
                            </button>
                          )}
                        </div>
