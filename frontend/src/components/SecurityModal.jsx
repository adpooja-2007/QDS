...
export default function SecurityModal({ isOpen, onClose, securityData, currentSender, activeRecipient }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#FCFBF8] border border-[#DDD4C5] rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-[#EAE3DA] bg-[#F7F3EC] flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 rounded-full hover:bg-[#E8E1D5] text-[#746D62] transition" title="Close (Esc)">
        ...
        <div className="p-5 space-y-4 text-xs font-sans max-h-[80vh] overflow-y-auto">
          <div className="space-y-2">
            <div className="flex items-center justify-between p-3 rounded-xl bg-[#F4EFEA] border border-[#E5DEC7]">
              <div>
              ...
              {passed ? (
                <span className="font-mono text-xs text-emerald-800 font-bold bg-emerald-100/90 px-2.5 py-1 rounded-lg border border-emerald-300">
                  ✓ VERIFIED (ACCEPT)
                </span>
              ) : (
                <span className="font-mono text-xs text-red-800 font-bold bg-red-100 px-2.5 py-1 rounded-lg border border-red-300">
                  ❌ COMPROMISED (REJECT)
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="p-3 rounded-xl bg-[#F4EFEA] border border-[#E5DEC7]">
                ...
                <span className={`font-mono text-sm font-bold mt-0.5 block ${passed ? 'text-emerald-700' : 'text-red-600'}`}>
                  {qberVal.toFixed(2)}%
                </span>
              ...
              <div className="p-3 rounded-xl bg-[#F4EFEA] border border-[#E5DEC7]">
                ...
                <span className={`font-mono text-sm font-bold mt-0.5 block ${chshVal >= 2.0 ? 'text-emerald-700' : 'text-red-600'}`}>
                  S = {chshVal.toFixed(2)}
                </span>
            ...
            <div className="p-3 rounded-xl bg-[#F4EFEA] border border-[#E5DEC7] space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] font-mono text-emerald-800 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">OPTIMAL</span>
              </div>
          ...
          <div className="border border-[#EAE3DA] rounded-xl overflow-hidden">
            <button
              ...
              className="w-full p-3 bg-[#F4EFEA] hover:bg-[#EAE3DA] text-left font-mono text-[11px] font-semibold text-[#332E27] flex justify-between items-center transition"
            >
              <span>🔬 View Raw Telemetry Proofs</span>
              <span>{showTechDetails ? '▲' : '▼'}</span>
            </button>
            {showTechDetails && (
              <div className="p-3.5 bg-white font-mono text-[10px] text-[#443E35] space-y-1.5 border-t border-[#EAE3DA]">
        ...
        <div className="p-3.5 border-t border-[#EAE3DA] bg-[#F7F3EC] flex justify-between items-center">
          ...
          <button onClick={onClose} className="bg-[#181B20] hover:bg-black text-cream-100 px-4 py-1.5 rounded-lg text-xs font-mono transition shadow-sm">
            Done
          </button>
        </div>
      </div>
