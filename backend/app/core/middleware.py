...
class TelemetryMiddleware(BaseHTTPMiddleware):
    ...
    def _save_telemetry_to_db(entry: TelemetryEntry) -> None:
        """Persist telemetry record to database store if DB is enabled."""
        try:
            from app.services.session_service import SessionService
            if not SessionService._db_enabled:
                return  # Skip blocking TCP attempts when running in SQLite in-memory fallback

            from sqlalchemy.orm import Session as SyncSession
            if SessionService._sync_engine:
                with SyncSession(SessionService._sync_engine) as db:
