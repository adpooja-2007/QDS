...
try {
  const backendPortPath = path.resolve(import.meta.dirname, "..", "backend_port.json");
  if (fs.existsSync(backendPortPath)) {
    const data = JSON.parse(fs.readFileSync(backendPortPath, "utf-8"));
    if (data.url) backendTarget = data.url;
    else if (data.port) backendTarget = `http://127.0.0.1:${data.port}`;
  }
} catch (e) {
  console.warn("Could not read backend_port.json, falling back to 8000", e);
}
  ...
  server: {
    ...
    proxy: {
      "/api": {
        target: (() => {
          try {
            const backendPortPath = path.resolve(PROJECT_ROOT, "..", "backend_port.json");
            if (fs.existsSync(backendPortPath)) {
              const data = JSON.parse(fs.readFileSync(backendPortPath, "utf-8"));
        ...
        configure: (proxy) => {
          proxy.on("error", (err, _req, res) => {
            ...
            if (res && typeof (res as any).writeHead === "function" && !(res as any).headersSent) {
              ...
              (res as any).end(JSON.stringify({ error: "Backend proxy connection failed", detail: err.message }));
            }
          });
        },
      },
