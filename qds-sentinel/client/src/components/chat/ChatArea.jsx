              ...
              >
                ...
                <div className={`flex flex-col min-w-0 max-w-full ${isOutgoing ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`mt-1.5 flex items-center gap-2 font-mono text-[10px] text-[#867E73] ${isOutgoing ? 'justify-end pr-1' : 'justify-start pl-1'}`}>
                    <span>{timeStr}</span>
                    {isOutgoing && (
                      <span className="inline-flex items-center gap-0.5 text-[11px] font-bold" title={msg.is_pending ? 'Transmitting photon stream...' : msg.is_read ? 'Read by recipient node' : 'Delivered to network'}>
                        {msg.is_pending ? (
                          <svg className="w-3 h-3 text-[#8C8479] animate-pulse inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <span className="text-[#867E73]">✓</span>
                        )}
                      </span>
                    )}
                    <button
