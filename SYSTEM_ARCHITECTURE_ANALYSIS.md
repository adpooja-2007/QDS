...
# 6. Tri-Dashboard Frontend Architecture (Kinetic Precision v2.0)

The system UI comprises 3 decoupled, institutional web applications running on independent ports:

1. **Dashboard 1: Cyber-SOC Sentinel (`q-email`, Port 3000)**:
   - `/home`: Central Gateway hub connecting to visualizer and monitoring.
   - `/demonstration`: Interactive Alice ↔ Bob quantum protocol simulator with 6-phase step runner, photon wavepacket animation, 35% Eve MitM toggle, and Pauli alignment matrix.
   - `/monitoring`: Dedicated SOC operations console with `overview`, `threats`, `incidents`, `sessions`, and `network` subdirectories.

2. **Dashboard 2: Red Team Attack Sandbox (`frontend`, Port 5173)**:
   - Interactive attack launcher (Clean, MitM 35%, Forgery, Replay, Noise, PNS).
   - 4-way terminal feeds (`Arbitrator.sys`, `Alice Node`, `Bob Node`, `Eve Intercept`).
   - Real-time QBER vs Hoeffding threshold & CHSH Bell violation graphs.

3. **Dashboard 3: Database Live Inspector (`db-dashboard`, Port 4000)**:
   - Schema & table switcher with live row counts (`quantum_sessions`, `node_telemetry`, `auth_logs`, `crypto_keys`, `vw_active_threats`).
   - Live query bar and spreadsheet grid auto-syncing every 2.5s.
   - JSONB slide-over bitstream drawer.
